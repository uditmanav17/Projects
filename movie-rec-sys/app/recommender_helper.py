...
def content_movie_recommender(
    ...
    try:
        ...
        movie_sim = similarity_db[similarity_db.index == input_movie].values[0]

        # get movies sorted by similarity
        sorted_movie_ids = np.argsort(movie_sim)[::-1]
        recommended_movies = movie_db_list[sorted_movie_ids[1 : top_n + 1]]
...
def get_popularity_rmse(
    ...
    if not filtered_df.empty:
        ...
        squared_diffs = (sample_movie_popularity - recommendations_popularity) ** 2  # noqa E501
...
def get_vote_avg_rmse(
    ...
    popularity_rmse : float
        The RMSE for popularity.
    """
    sample_movie_vote_average = df[df["title"] == sample_movie].vote_average.iloc[0]  # noqa E501
...
def get_vote_count_rmse(
    ...
    -------
        ...
        The RMSE for popularity.
    """
    sample_movie_popularity = df[df["title"] == sample_movie].vote_count.iloc[0]  # noqa E501
...
def create_combined(df: pd.DataFrame, weight: int = 2) -> pd.DataFrame:
    """
...
def compute_tfidf_vectorization(df: pd.DataFrame, stop_words: str = "english"):
    """
